...
pub fn load_from(lookup: impl Fn(&str) -> Option<String>) -> CliEnvValues {
    CliEnvValues {
        api_base: lookup("ORES_OTEL_API_BASE").filter(|value| !value.is_empty()).unwrap_or_else(|| "http://127.0.0.1:8080".to_string()),
        env_map_probe: lookup("ENV_MAP_PROBE").filter(|value| !value.is_empty()),
        json: parse_bool(lookup("ORES_OTEL_JSON"), false),
    }
}
...
pub fn load_env_map(
    ...
    let mut out = std::collections::BTreeMap::new();
    let api_base = pick(&["ORES_OTEL_API_BASE"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, Some("http://127.0.0.1:8080"));
    if let Some(value) = api_base {
        out.insert("ORES_OTEL_API_BASE".to_string(), value);
    }
    ...
    let json = pick(&["ORES_OTEL_JSON"], &["flags", "env_shell", "env_file"], shell, dotenv, flags, Some("false"));
    if let Some(value) = json {
        out.insert("ORES_OTEL_JSON".to_string(), value);
    }
    Ok(out)
}
