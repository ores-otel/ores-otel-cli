...
export function loadFrom(lookup: (key: string) => string | undefined): CliEnvValues {
  return {
    api_base: nonEmpty(lookup("ORES_OTEL_API_BASE")) ?? "http://127.0.0.1:8080",
    env_map_probe: nonEmpty(lookup("ENV_MAP_PROBE")),
    json: parseBool(lookup("ORES_OTEL_JSON"), false),
  };
}
...
export function loadEnvMap(
  ...
  const out: Record<string, string> = {};
  const api_base = pick(["ORES_OTEL_API_BASE"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, "http://127.0.0.1:8080");
  if (api_base !== undefined) out["ORES_OTEL_API_BASE"] = api_base;
  ...
  const json = pick(["ORES_OTEL_JSON"], ["flags", "env_shell", "env_file"], shell, dotenv, flags, "false");
  if (json !== undefined) out["ORES_OTEL_JSON"] = json;
  return out;
}
